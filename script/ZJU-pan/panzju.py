...
def upload(
    token, filename, data, filesize=None, folder_id=0, retry=5, fencrypt=None, _a=None
):
    """
    ...
    x = a.post(
        DOMAIN + "/apps/files/presign_upload",
        json.dumps({"folder_id": folder_id, "file_size": filesize}),
        headers={
            "requesttoken": token,
    ...
    if result.get("success") != True:
        # 兼容API响应变化：如果没有success字段，尝试直接使用upload_url
        if "upload_url" in result:
            upload_url = result["upload_url"]
        else:
            print("presign_upload response:", result)
            print("x.text:", x.text)
            return False
    else:
        upload_url = result["upload_url"]
...
def share(token, fileid):
    ...
    global a
    data = (
        """{"access": "public", "disable_download": "0", "due_time": "never_expire", "password_protected": false,"item_typed_id": "%s"}"""
        % fileid
    )
    x = a.post(
        DOMAIN + "/apps/files/share",
        data,
        headers={
            "requesttoken": token,
            "X-Requested-With": "XMLHttpRequest",
...
def getshareid(folder_id):
    ...
    global a
    x = a.get(
        "{DOMAIN}/apps/files/get_share_info?item_typed_id=folder_{folder_id}".format(
            DOMAIN=DOMAIN, folder_id=folder_id
        ),
        result=False,
        o=True,
    )
...
def download(file_uniqe_name):
    ...
    fileid = tmp.group(1)
    x = a.get(
        DOMAIN + "/apps/files/download?file_id={}&scenario=share".format(fileid),
        o=True,
        result=False,
    )
...
def logined_download(sharelink, a):
    ...
    if "Location" in x.headers:
        ...
        if "login" in x.headers["Location"]:
            ...
            x = a.get(
                "{DOMAIN}/apps/files/download?file_id={fileid}".format(
                    DOMAIN=DOMAIN, fileid=fileid
                ),
                o=True,
                result=False,
            )
...
def mkdir(token, name, parent_id=0):
    ...
    data = ("""{"parent_folder_id":%s,"name":"%s"}""" % (str(parent_id), name)).encode(
        "utf-8"
    )
    x = a.post(
        DOMAIN + "/apps/files/new_folder",
        data,
        headers={
            "requesttoken": token,
            "x-requested-with": "XMLHttpRequest",
    ...
    if "success" not in data or data["success"] != True:
        ...
        raise FileExistsError(
            "failed to create {name} under folder_{parent_id}. data={data}".format(
                **locals()
            )
        )
...
def file_info(token, typed_id):
    ...
    global a
    x = a.get(
        DOMAIN + "/apps/files/get_info?item_typed_id=" + typed_id,
...
def lsdir(
    ...
    data = a.get(
        DOMAIN
        + "/apps/files/file_list/{folder_id}?page_number={pageid}&{from_share_string}page_size=100".format(
            pageid=pageid, folder_id=folder_id, from_share_string=from_share_string
        ),
        result=False,
        o=True,
    ).json()
        ...
        if pageid > 1:
            data = a.get(
                ...
                + "/apps/files/file_list/{folder_id}?page_number={pageid}&{from_share_string}page_size=100".format(
                    pageid=pageid,
                    folder_id=folder_id,
                    from_share_string=from_share_string,
                ),
                result=False,
                o=True,
            ).json()
...
def path_to_typed_id(filepath, cache=None, onerror="raise"):
    ...
    if cache is not None:
        ...
        assert not filepath.startswith(
            "/"
        ), "cache not matched: filepath='%s', while cache['path']='%s'" % (
            filepath,
            cache["path"],
        )
...
def generate_fscache(
    path, fid=None, onlydirs=False, prefix=None, only1depth=False, cache=None
):
    """
    ...
    if prefix is None:
        prefix = ""
    if fid is None:
        typetmp, fid = path_to_typed_id(
            path, cache=cache, onerror="when_cache_fails_continue_search_by_using_lsdir"
        )
    ...
    for item in lsdir(a, fid, onlydirs=onlydirs):
        ...
        if type == "folder":
            ...
            if not only1depth:
                fs.update(
                    generate_fscache(
                        "", fileid, onlydirs=onlydirs, prefix=prefix + item[0] + "/"
                    )
                )
        elif type == "file":
    ...
    if prefix != "":  # 用于递归的返回，只返回文件系统部分
        ...
        if not path.endswith("/"):
            path += "/"
        result = {"fs": fs, "path": path, "fid": fid}
...
def mkdir_and_update_cache(token, name, parent_relative_path, cache, onerror="raise"):
    ...
    try:
        ...
        if onerror == "raise":
            ...
            _, fileid, size = lsdir(
                a, parent_id, onlyneed=name, onlyneed_return_size=True
            )
...
def mkdir_p(token, path, cache):
    ...
    for name in path.split("/"):
        ...
        if key[1:] in cache["fs"]:
            ...
            assert (
                type == "folder"
            ), "there is a file {cache_path}{key} in the way".format(
                cache_path=cache["path"], key=key[1:]
            )
        else:
            try:
                ...
                type, fid, size = lsdir(
                    a, fid, onlyneed=name, onlyneed_return_size=True
                )
...
def fillup_cache(abs_path, cache=None):
    ...
    for i in range(len(path_splited)):
        ...
        cache["fs"].update(
            generate_fscache(
                "/" + nowpath, prefix=nowpath + "/", only1depth=True, cache=cache
            )
        )
    return cache


def upload_directory_targetid(
    token,
    local_dir,
    target_folder_id,
    skip_existed=False,
    ...
    # 检查本地目录要是一个目录
    global a
    assert os.path.isdir(local_dir), "expected a folder, local_dir={local_dir}".format(
        **locals()
    )
    ...
    try:
        ...
        targetfid = [
            i[1].split("_")[1]
            for i in target_folder_lsdir
            if i[0] == name and i[1].split("_")[0] == "folder"
        ][0]
    print(targetfid)
    ...
    for root, dirs, files in os.walk(local_dir):
        for dir in dirs:
            dirname = (
                root.replace(local_dir, "", 1).replace("\\", "/") + "/" + dir
            )  # "/Image/aha"
            mkdir_p(token, target_folder_path + dirname, cache)
        for filename in files:
            relative_root = root.replace(local_dir, "", 1).replace(
                "\\", "/"
            )  # "/Image/aha"或者""
            remote_abs_folder = (
                target_folder_path + relative_root
            )  # "uploaded/Image/aha"或者"uploaded" 注意虽然叫做abs实际上还是相对于cache["path"]的相对目录
            remote_abs_filepath = (
                remote_abs_folder + "/" + safefilename(filename)
            )  # "uploaded/Image/aha/example.jpg"或者"uploaded/example.jpg"
            # print(remote_abs_folder, cache)
            type, folder_id = path_to_typed_id(remote_abs_folder, cache)
            assert type == "folder", "expected folder {remote_abs_folder}".format(
                **locals()
            )
            ...
            newfileid = upload(
                token, filename, data, filesize, folder_id=folder_id, fencrypt=fencrypt
            )
...
def upload_directory(
    ...
    assert not target_folder_path.startswith(
        "/"
    ), "cache not matched! path={target_folder_path}".format(**locals())
    assert not target_folder_path.endswith(
        "/"
    ), "target_folder_path should not end with /"
    ...
    cache["fs"].update(
        generate_fscache("", fid=fid, prefix=target_folder_path + "/", cache=cache)
    )
        ...
        for filename in files:
            ...
            if fencrypt.get("fencrypt_filename", None) is not None:
                remote_abs_filepath = (
                    remote_abs_folder
                    + "/"
                    + fencrypt["fencrypt_filename"](safefilename(filename))
                )  # filename encrypted
            else:
                remote_abs_filepath = (
                    remote_abs_folder + "/" + safefilename(filename)
...
def upload_by_collection(upload_link, path_to_file):
    ...
    return upload_by_collection_detailed(
        token,
        upload_link,
        filename,
        getsize(path_to_file),
        block(open(path_to_file, "rb")),
    )
...
def upload_by_collection_detailed(token, collection_code, filename, filesize, data):
    ...
    global a
    x = a.post(
        DOMAIN + "/apps/processes/presign_upload",
        (
            """{"code":"%s","file_name":"%s","file_size":%d}"""
            % (collection_code, filename, filesize)
        ).encode(
            "utf-8"
        ),  # 在格式化字符串之后再进行编码
        headers={
    ...
    x = a.post(
        DOMAIN + "/apps/processes/collection_submit",
...
def dirshare_download(share_link, fileid):
    ...
    x = a.get(
        DOMAIN + "/apps/files/download?file_id={}&scenario=share".format(fileid), o=True
    )
    assert (
        x.status_code == 302
    ), "download failed, please check misspelling or privilege setting"
    return x.headers["Location"]
...
def copy_from_share(token, sharelink, targetid, _a=None):
    ...
    if "Location" in x.headers:
        ...
        fileid = tmp.group(1)
    postdata = (
        """{"target_folder_id":%s,"item_typed_ids":"file_%s","scenario":"share"}"""
        % (str(targetid), fileid)
    )
    x = a.post(
        DOMAIN + "/apps/files/copy?scenario=share",
        postdata,
        headers={"Content-Type": "text/plain;charset=UTF-8", "requesttoken": token},
    )
    try:
        data = x.json()
...
def changename(a, token, newname):
    ...
    return a.post(
        DOMAIN + "/user_settings/update",
        postdata,
        headers={
            "requesttoken": token,
            "Content-Type": "application/json",
            "Referer": DOMAIN + "/user_settings/index",
        },
    )
    ...
    if not token:
        if username is None:
            try:
                ...
                raise Exception(
                    "please set username password in config.py, or pass it as parameter"
                )
...
def UI_login_upload(username=None, password=None):
    ...
    if not token:
        if username is None:
            if len(sys.argv) < 4:
                print(
                    "Usage: python3 {} filename username password".format(sys.argv[0])
                )
                exit(1)
            username = sys.argv[2]
            password = sys.argv[3]
