...
class EasyLogin:
    ...
    def get(
        ...
        **kwargs
        ...
        if cache is not None and os.path.exists(
            cache_filepath
        ):  # cache exist, read from cache
            ...
            if result:
                ...
                if encoding:
                    self.b = BeautifulSoup(
                        page.decode(encoding, errors="replace"), "html.parser"
                    )
                else:
    ...
    def post_dict(self, url, dict, result=True, save=False, headers=None, cache=None):
        ...
        data = urlencode(dict)
        return self.post(
            url, data, result=result, save=save, headers=headers, cache=cache
        )

    def post_json(
        self, url, jsondata, result=False, save=False, headers=None, cache=None, o=False
    ):
        """
    ...
    def getlist(
        self, searchString, elementName="a", searchTarget="href", returnType=None
    ):
        """
        ...
        for element in self.b.find_all(elementName):
            if searchString in element.get(searchTarget, ""):
                result.append(
                    element[returnType] if returnType != "element" else element
                )
        return result
    ...
    def css(self):
        return [
            i[2:] if i[0:2] == "//" else i for i in self.getlist("css", "link", "href")
        ]

    def js(self):
        return [
            i[2:] if i[0:2] == "//" else i for i in self.getlist("js", "script", "src")
        ]
    ...
    def text(self, target=None, ignore_pureascii_words=False):
        ...
        for descendant in target.descendants:
            if (
                not isinstance(descendant, NavigableString)
    ...
    def find(self, tag, attrs_string, skip=0, text=False):
        ...
        """
        tmp_tag = BeautifulSoup(
            """<%s %s></%s>""" % (tag, attrs_string, tag), "html.parser"
        ).find(tag)
