...
#let winnote(
  ...
  show outline.entry.where(level: 1): it => {
    text(font: winnote-font)[#strong[#it]]
  }
  set par(leading: 9pt) //目录行间距
  show outline.entry: it => {
    text(font: winnote-font)[#it]
  }
  ...
  align(center)[
    #set text(font: winnote-font)
    #block(text(size: 36pt, fill: green.B, title))
    #v(1em, weak: true)
  ]

  align(bottom + center)[
    #set text(size: 15pt, fill: blue.B)
    #author
  ]

  align(bottom + center)[
    #set text(size: 15pt, fill: orange.B)
    #createtime
  ]
  ...
  if preface != none {
    ...
    align(center)[
      #set text(font: winnote-font, size: 20pt)
      *#if lang == "en" [Preface] else [序]*
    ]
    [
      #set text(font: winnote-font, size: 12pt)
      #preface
    ]
  }
  ...
  set figure(
    numbering: (..nums) => (
      context {
        set text(font: winnote-font, size: 9pt)
        numbering("1.1", chaptercounter.at(here()).first(), ..nums)
      }
    ),
  )
  ...
  //图片表格 caption 字体字号
  show figure.caption: set text(font: winnote-font, size: 9pt)
  ...
  if bibliography-file != none {
    set text(font: winnote-font) //设置参考文献字体
    pagebreak()
